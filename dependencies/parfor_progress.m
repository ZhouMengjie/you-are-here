function percent = parfor_progress(processName, N)
% Changed to include processName
...
elseif N == 0
    ...
    if nargout == 0
        disp([repmat(char(8), 1, (w+9+w_addon)), char(10), processName, '100%[', repmat('=', 1, w+1), ']']);
    end
else
    ...
    if nargout == 0
        ...
        disp([repmat(char(8), 1, (w+9+w_addon)), char(10), processName, ' ', perc, '[', repmat('=', 1, round(percent*w/100)), '>', repmat(' ', 1, w - round(percent*w/100)), ']']);
    end
end
